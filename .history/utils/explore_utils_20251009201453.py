...
def show_summary_statistics(df: pd.DataFrame):
    """Show summary statistics and robust Plotly histogram for selected column."""
    st.subheader("Summary Statistics")
    st.dataframe(df.describe(include="all").T)

    # --- Identify numeric columns ---
    numeric_cols = df.select_dtypes(include=["float", "int", "float64", "int64"]).columns.tolist()
    ...
    plot_df[selected_col] = plot_df[selected_col].astype(float)
    x_values = plot_df[selected_col].tolist()

    # --- Create Plotly histogram (robustly) ---
    fig = go.Figure()
    ...
    # Box plot overlay (marginal effect)
