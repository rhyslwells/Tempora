...
def show_data_summary(df: pd.DataFrame):
    ...
    with col1:
        ...
        st.write("**Columns:**")
        st.write(list(df.columns))  # show columns in readable format
    with col2:
        st.write("**Data types:**")
        # FIX: Convert dtypes to a proper DataFrame with string types
    ...
    if missing.sum() > 0:
        with st.expander("Missing Values"):
            # FIX: Convert to proper DataFrame
    ...
    if selected_columns:
        ...
        if len(numeric_cols) > 0:
            with st.expander("Basic Statistics (Numeric Columns)"):
                # FIX: Reset index to avoid Arrow issues
