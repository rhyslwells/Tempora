...
def app():
    ...
    if detrend_method == "First Difference (df.diff())":
        df_transformed = df[numeric_cols].diff()
    elif detrend_method == "Second Difference (df.diff().diff())":
        df_transformed = df[numeric_cols].diff().diff()
    elif detrend_method == "Remove Linear Trend":
        # Keep as DataFrame with correct index and column name
        df_transformed = pd.DataFrame(
            detrend(df[numeric_cols], type='linear'),
            index=df.index,
