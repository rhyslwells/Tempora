...
def app():
    ...
    # Subset the dataframe to the selected column
    df = df[[selected_col]]
    numeric_cols = df.select_dtypes(include=["float64", "int64"]).columns
    non_numeric_cols = set([selected_col]) - set(numeric_cols)
    if non_numeric_cols:
        st.warning(f"Column {non_numeric_cols} is non-numeric and cannot have log/differencing applied.")
    ...
    if not plot_df.empty:
        col_to_plot = plot_df.columns[0]
        fig = px.line(
            plot_df,
            x=plot_df.index,
            y=col_to_plot,
            title=f"Time Series Preview: {col_to_plot}",
            markers=True
        )
