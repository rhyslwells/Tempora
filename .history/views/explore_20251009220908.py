...
def plot_time_series(df: pd.DataFrame, col: str, date_col: str):
...
def show_summary_statistics(df: pd.DataFrame):
    ...
    # Use the single numeric column in session_state if it exists
    target_col = st.session_state.get("selected_col", numeric_cols[0])
...
def show_time_series_view(df: pd.DataFrame):
    ...
    if date_col is None or target_col is None:
        st.warning("⚠️ Date column or numeric column not set.")
    ...
    plot_time_series(df_copy, target_col, date_col)
    return date_col, target_col
...
def show_moving_average(df: pd.DataFrame):
    ...
    target_col = st.session_state.get("selected_col", None)
    if date_col is None or target_col is None:
        st.info("Please select the date and numeric column first.")
        return
...
def show_decomposition(df: pd.DataFrame):
    ...
    try:
        ...
        fig = make_decomposition_plot(df_copy[date_col], result, target_col)
        st.plotly_chart(fig, use_container_width=True)
...
def app():
    ...
    target_col = st.session_state["selected_col"]
    ...
    with tab1:
        # Ensure Date column is available as a column
        df_copy = df.copy()
        if date_col not in df_copy.columns:
            # date_col might be the index
            df_copy = df_copy.reset_index()
            if date_col not in df_copy.columns:
                st.error(f"Date column '{date_col}' not found in the DataFrame!")
                return
